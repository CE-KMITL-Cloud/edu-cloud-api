...
func Start(c *fiber.Ctx) error {
	...
	if startErr != nil {
		log.Printf("Error: Could not starting VMID : %s in %s : %s", vmid, node, startErr)
		return startErr
	}
	...
	log.Printf("Error: Could not starting VMID : %s in %s", vmid, node)
